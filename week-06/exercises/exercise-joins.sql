...
where c.last_name like 'D%' or c.last_name like 'B%';
...
select *
from employee e
left outer join project_employee p on e.employee_id = p.employee_id
where p.project_id is null;
...
p.description
from employee e
left outer join project_employee x on e.employee_id = x.employee_id
left outer join project p on x.project_id = p.project_id
...
p.project_id
from project p
left outer join project_employee x on p.project_id = x.project_id
left outer join employee e on x.employee_id = e.employee_id
...
p.project_id,
i.name,
pi.quantity * i.price_per_unit as total
from customer c
left outer join project p on c.customer_id = p.customer_id
